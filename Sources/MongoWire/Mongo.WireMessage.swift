...
extension Mongo {
    @frozen public struct WireMessage: Sendable {
        public let header: WireHeader
        public let flags: WireFlags
        public let sections: Sections
        public let checksum: CRC32?
...
extension Mongo.WireMessage: Identifiable {
    @inlinable public var id: Mongo.WireMessageIdentifier { self.header.id }
}
extension Mongo.WireMessage {
    @inlinable public init(
        sections: Sections,
        checksum: Bool = false,
        id: Mongo.WireMessageIdentifier
    ) {
        ...
        self.init(
            header: .init(count: count, id: id), flags: flags,
            sections: sections,
            checksum: crc32
        )
    }
...
extension Mongo.WireMessage {
    @inlinable public static func += (output: inout some BSON.OutputStream, self: Self) {
        output += self.header
