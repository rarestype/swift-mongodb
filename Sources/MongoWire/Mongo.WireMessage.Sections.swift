...
extension Mongo.WireMessage {
    @frozen public struct Sections: Sendable {
        public let body: BSON.Document
        public let outlined: [Outline]

        @inlinable public init(body: BSON.Document, outlined: [Outline] = []) {
            self.body = body
...
extension Mongo.WireMessage.Sections {
    @inlinable internal static func parse(from input: inout BSON.Input) throws -> Self {
        var body: BSON.Document? = nil
        var outlined: [Mongo.WireMessage.Outline] = []

        while let section: UInt8 = input.next() {
            guard
            let section: Mongo.WireSection = .init(rawValue: section) else {
