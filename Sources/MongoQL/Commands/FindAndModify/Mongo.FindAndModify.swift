...
extension Mongo {
    public struct FindAndModify<Effect>: Sendable where Effect: Mongo.ModificationEffect {
...
extension Mongo.FindAndModify: Mongo.Command {
    @inlinable public static var type: Mongo.CommandType { .findAndModify }

    public typealias ExecutionPolicy = Mongo.Retry

    public typealias Response = (value: Effect.Value, upserted: Effect.ID?)
...
extension Mongo.FindAndModify {
    public init(
        ...
        self.init(
            writeConcern: writeConcern, fields: Self.type(collection) {
...
extension Mongo.FindAndModify {
    @frozen public enum Fields: String, Hashable, Sendable {
        case fields
    }
    ...
    @inlinable public subscript<ProjectionDocument>(key: Fields) -> ProjectionDocument?
...
extension Mongo.FindAndModify: Mongo.PredicateConfigurable {
    @frozen public enum Query: String, Hashable, Sendable {
        case query
    }
...
extension Mongo.FindAndModify where Effect.Upsert == Bool {
    @frozen public enum Update: String, Hashable, Sendable {
        case update
    }

    @inlinable public subscript(key: Update, yield: (inout Mongo.UpdateEncoder) -> ()) -> Void {
        mutating get {
    ...
    @inlinable public subscript(
        key: Update,
        yield: (inout Mongo.PipelineEncoder) -> ()
    ) -> Void {
        mutating get {
    ...
    @inlinable public subscript<Replacement>(key: Update) -> Replacement?
