...
extension Mongo {
    /// Collection options.
    @frozen public struct CollectionOptions: Sendable {
        public let collation: Collation?
        public let writeConcern: WriteConcern.Options?
        public let indexOptionDefaults: BSON.Document?
        public let storageEngine: BSON.Document?
...
extension Mongo.CollectionOptions {
    @inlinable public var capped: Bool {
    ...
    @inlinable public var validationAction: Mongo.ValidationAction? {
        switch self.variant {
        case .collection(
                cap: _,
    ...
    @inlinable public var validationLevel: Mongo.ValidationLevel? {
        switch self.variant {
    ...
    @inlinable public var timeseries: Mongo.Timeseries? {
        self.variant.timeseries
    }
    @inlinable public var viewOn: Mongo.Collection? {
        self.variant.view?.collection
    }
    @inlinable public var pipeline: Mongo.Pipeline? {
        self.variant.view?.pipeline
    }
}
extension Mongo.CollectionOptions {
    @inlinable public init(
        bson: BSON.DocumentDecoder<BSON.Key>,
        type: Mongo.CollectionType
    ) throws {
        let variant: Variant
        switch type {
        case .collection:
