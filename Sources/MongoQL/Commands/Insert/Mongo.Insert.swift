...
extension Mongo {
    ...
    public struct Insert: Sendable {
        ...
        @usableFromInline internal var documents: BSON.Output
...
extension Mongo.Insert: Mongo.Command {
    @inlinable public static var type: Mongo.CommandType { .insert }

    /// `Insert` supports retryable writes.
    public typealias ExecutionPolicy = Mongo.Retry

    public typealias Response = Mongo.InsertResponse
...
extension Mongo.Insert {
    @inlinable public init(
        ...
        documents encode: (inout Mongo.InsertListEncoder) throws -> ()
    ) rethrows {
    ...
    @inlinable public init<Elements>(
        _ collection: Mongo.Collection,
        writeConcern: Mongo.WriteConcern? = nil,
