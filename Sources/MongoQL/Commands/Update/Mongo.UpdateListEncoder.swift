...
extension Mongo {
    @frozen public struct UpdateListEncoder<Effect> where Effect: Mongo.WriteEffect {
...
extension Mongo.UpdateListEncoder {
    @inlinable public mutating func callAsFunction<T>(
        with yield: (inout Mongo.UpdateStatementEncoder<Effect>) throws -> T
    ) rethrows -> T {
        try yield(
            &self.output[
...
extension Mongo.UpdateListEncoder {
    ...
    @inlinable public mutating func upsert(
        _ element: some BSONDocumentEncodable & Identifiable<some BSONEncodable>
    ) {
