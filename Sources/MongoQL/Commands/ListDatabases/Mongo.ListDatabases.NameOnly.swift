...
extension Mongo.ListDatabases {
    ...
    public struct NameOnly: Sendable {
...
extension Mongo.ListDatabases.NameOnly: Mongo.Command {
    ...
    public typealias ExecutionPolicy = Mongo.ListDatabases.ExecutionPolicy

    /// ``ListDatabases`` must be run against the `admin` database.
    public typealias Database = Mongo.ListDatabases.Database

    public typealias Response = [Mongo.Database]

    @inlinable public static func decode(
        reply bson: BSON.DocumentDecoder<BSON.Key>
    ) throws -> [Mongo.Database] {
...
extension Mongo.ListDatabases.NameOnly {
    @inlinable public subscript(key: Mongo.ListDatabases.AuthorizedDatabases) -> Bool? {
        get {
            nil
        }
        set(value) {
    ...
    @inlinable public subscript(key: Mongo.ListDatabases.Filter) -> Mongo.PredicateDocument? {
        get {
            nil
        }
        set(value) {
