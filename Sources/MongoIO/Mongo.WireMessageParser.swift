...
extension Mongo {
    public final class WireMessageParser {
        private var header: Mongo.WireHeader?
...
extension Mongo.WireMessageParser {
    private func read(until count: Int, from buffer: inout ByteBuffer) -> Bool {
...
extension Mongo.WireMessageParser: ChannelInboundHandler {
    ...
    public typealias InboundOut = Mongo.WireMessage
    ...
    public func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        do {
            ...
            while 0 < incoming.readableBytes {
                switch self.header {
                case nil:
                    guard self.read(until: Mongo.WireHeader.size, from: &incoming) else {
                        return
                    }
                ...
                case let header?:
                    guard self.read(until: header.count, from: &incoming) else {
                        return
                    }
