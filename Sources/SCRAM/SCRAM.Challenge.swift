...
extension SCRAM {
    @frozen public struct Challenge {
        public let iterations: Int
        public let nonce: Nonce
        public let salt: [UInt8]

        private init(iterations: Int, nonce: Nonce, salt: [UInt8]) {
...
extension SCRAM.Challenge {
    @inlinable public func id(password: String) -> CacheIdentifier {
        .init(iterations: self.iterations, password: password, salt: self.salt)
    }
}
extension SCRAM.Challenge {
    public init(from message: SCRAM.Message) throws {
