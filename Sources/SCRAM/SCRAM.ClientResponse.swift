...
extension SCRAM {
    ...
    @frozen public struct ClientResponse<Hash> where Hash: MessageAuthenticationHash {
        public let message: Message
        @usableFromInline let signature: Hash

        @usableFromInline init(message: Message, signature: Hash) {
...
extension SCRAM.ClientResponse {
    @inlinable public init(
        ...
        guard challenge.nonce.string.starts(with: sent.nonce.string) else {
        ...
        self.init(
            message: .init("\(prefix),p=\(Base64.encode(proof))"),
...
extension SCRAM.ClientResponse {
    ...
    @inlinable public func verify(_ response: SCRAM.ServerResponse) -> Bool {
