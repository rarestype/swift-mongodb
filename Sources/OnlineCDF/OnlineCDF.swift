...
public struct OnlineCDF: Sendable {
    ...
    private init(
        resolution: Int, centroids: [Centroid],
        weight: Double,
        min: Double,
        max: Double,
        sum: Double
...
extension OnlineCDF {
    private init(
        ...
        self.init(
            resolution: resolution, centroids: [],
            weight: weight,
            min: min,
            max: max,
            sum: 0
...
extension OnlineCDF {
    public init(resolution: Int, seed: Double) {
        let seed: Centroid = .init(weight: 1, mean: seed)
        self.init(
            resolution: resolution, centroids: [seed],
            weight: seed.weight,
            min: seed.mean,
            max: seed.mean,
            sum: seed.sum
    ...
    public init(resolution: Int, sorted: [Double]) {
        if sorted.isEmpty {
            fatalError("CDF seed values list cannot be empty.")
        }

        self.init(
            resolution: resolution, weight: 0,
            min: .infinity,
            max: -.infinity
        )
        self.insert(sorted: sorted)
    }
    @inlinable public init(resolution: Int, seeds: some Sequence<Double>) {
...
extension OnlineCDF {
    public mutating func insert(sorted: [Double]) {
        self = self.merged(with: sorted)
    }
    public mutating func insert(_ sample: Double) {
        self = self.merged(with: CollectionOfOne<Double>.init(sample))
    }
    @inlinable public mutating func insert(_ samples: some Sequence<Double>) {
        self.insert(sorted: samples.sorted())
    }
}
extension OnlineCDF {
    private func limit(k: Int) -> Double {
    ...
    private func merged<Samples>(with sorted: Samples) -> Self
...
extension OnlineCDF {
    public func estimate(quantile: Double) -> Double {
    ...
    private func snap(upper quantile: Double) -> (index: Int, fraction: Double) {
    ...
    private func snap(lower quantile: Double) -> (index: Int, fraction: Double) {
        let rank: Double = self.weight * quantile
