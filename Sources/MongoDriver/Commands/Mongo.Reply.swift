...
extension Mongo {
    @frozen public struct Reply {
        @usableFromInline internal let result: Result<BSON.DocumentDecoder<BSON.Key>, any Error>

        @usableFromInline internal let operationTime: BSON.Timestamp?
        @usableFromInline internal let clusterTime: ClusterTime?

        init(
            result: Result<BSON.DocumentDecoder<BSON.Key>, any Error>,
...
extension Mongo.Reply {
    ...
    @inlinable internal func callAsFunction() throws -> BSON.DocumentDecoder<BSON.Key> {
        try self.result.get()
    }
}
extension Mongo.Reply {
    public init(message: Mongo.WireMessage) throws {
        ...
        if  let code: Int32 = try bson["code"]?.decode() {
            ...
            self.init(
                result: .failure(Mongo.ReplyError.uncoded(message: message)),
                operationTime: operationTime,
                clusterTime: clusterTime
            )
            return
