...
extension Mongo {
    public struct RefreshSessions: Sendable {
        let sessions: [SessionIdentifier]

        public init(_ session: SessionIdentifier) {
            self.sessions = [session]
        }

        public init?(_ sessions: [SessionIdentifier]) {
            if  sessions.isEmpty {
                return nil
            } else {
...
extension Mongo.RefreshSessions: Mongo.Command, Mongo.ImplicitSessionCommand {
    ...
    @inlinable public static var type: Mongo.CommandType { .refreshSessions }
