extension Mongo.ReplicaSetConfiguration {
    @frozen public enum Rights: Equatable, Sendable {
...
extension Mongo.ReplicaSetConfiguration.Rights {
    ...
    @inlinable public static var citizen: Self {
        .citizen(.init())
    }
    /// Configures a resident with default settings.
    /// (Builds indexes, and is not delayed.)
    @inlinable public static var resident: Self {
        .resident(.init())
    }

    @inlinable public init(priority: Double) {
