...
extension Mongo {
    public struct ClusterTime: Sendable {
        public let timestamp: BSON.Timestamp
        let signature: BSON.Document

        @usableFromInline init(timestamp: BSON.Timestamp, signature: BSON.Document) {
...
extension Mongo.ClusterTime: BSONDecodable, BSONDocumentDecodable {
    @inlinable public init(bson: BSON.DocumentDecoder<CodingKey>) throws {
        self.init(
            timestamp: try bson[.clusterTime].decode(to: BSON.Timestamp.self),
