...
extension Mongo.ServerTable.Replicated {
    private init(
        ...
        if  let primary: Mongo.Server<Mongo.Replica> {
            ...
            let secondary: Mongo.Server<Mongo.Replica> = secondaries.max(
                by: {
    ...
    init(
        from topology: borrowing Mongo.Topology<Mongo.TopologyModel.Canary>.Replicated,
        heartbeatInterval: Milliseconds
    ) {
        ...
        for (host, state): (
            ...
            case .primary(let metadata):
                ...
                logicalSessionTimeoutMinutes = min(
        ...
        self.init(
            ...
            primary: primary,
            state: logicalSessionTimeoutMinutes == .max ? .unknown : .capable(
                .init(
                    transactions: true,
...
extension Mongo.ServerTable.Replicated {
    ...
    func nearest(
        by eligibility: Mongo.ReadPreference.Eligibility
