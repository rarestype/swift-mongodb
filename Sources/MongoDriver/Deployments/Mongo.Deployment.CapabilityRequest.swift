extension Mongo.Deployment {
    typealias CapabilityResponse = Result<
        Mongo.DeploymentCapabilities,
...
extension Mongo.Deployment.CapabilityRequest? {
    mutating func fail(diagnosing servers: Mongo.ServerTable) {
        guard let request: Wrapped = self else {
            return
        }
        defer {
            self = nil
    ...
    mutating func fulfill(with capabilities: Mongo.DeploymentCapabilities) {
