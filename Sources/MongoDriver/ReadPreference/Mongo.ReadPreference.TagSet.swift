...
extension Mongo.ReadPreference {
    ...
    @frozen public struct TagSet {
        ...
        public let patterns: [(key: BSON.Key, value: String)]

        @inlinable public init(patterns: [(key: BSON.Key, value: String)]) {
...
extension Mongo.ReadPreference.TagSet: ExpressibleByDictionaryLiteral {
    @inlinable public init(dictionaryLiteral: (BSON.Key, String)...) {
        self.init(patterns: dictionaryLiteral)
    }
}
extension Mongo.ReadPreference.TagSet {
    @inlinable public static func ~= (lhs: Self, rhs: [BSON.Key: String]) -> Bool {
        for (key, value): (BSON.Key, String) in lhs.patterns {
            guard case value? = rhs[key] else {
                return false
            }
        }
        return true
