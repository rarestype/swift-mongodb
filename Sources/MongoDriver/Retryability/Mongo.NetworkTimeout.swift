...
extension Mongo {
    @frozen @usableFromInline struct NetworkTimeout: Sendable {
        @usableFromInline let milliseconds: Milliseconds

        @inlinable init(milliseconds: Milliseconds) {
            self.milliseconds = milliseconds
        }
    }
}
extension Mongo.NetworkTimeout {
    @inlinable internal func deadline(
        from start: ContinuousClock.Instant = .now
    ) -> ContinuousClock.Instant {
        start.advanced(by: .init(self.milliseconds))
    }

    @inlinable internal func deadlines(
        from started: ContinuousClock.Instant = .now,
        clamping deadline: ContinuousClock.Instant?
    ) -> Mongo.Deadlines {
