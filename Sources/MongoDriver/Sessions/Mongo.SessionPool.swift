...
extension Mongo {
    ...
    /// cleanup on shutdown.
    public final actor SessionPool {
        ...
        /// ``Session`` objects.
        @usableFromInline nonisolated let deployment: Deployment
        ...
        private var requests: Deque<CheckedContinuation<Allocation, Never>>
        ...
        deinit {
            ...
            guard self.retained.isEmpty, self.released.isEmpty else {
                fatalError(
                    "unreachable (deinitialized session pool while pool contains sessions!)"
                )
            }
            guard case .draining = self.phase else {
                fatalError(
                    "unreachable (deinitialized session pool that has not been drained!)"
                )
            }
        }
    }
...
extension Mongo.SessionPool {
    public nonisolated func _servers() async -> [(host: Mongo.Host, latency: Nanoseconds)] {
        guard
        case .replicated(let servers) = await self.deployment.servers else {
            return []
        }
    ...
    public nonisolated var logger: Mongo.Logger? {
...
extension Mongo.SessionPool {
    ...
    public nonisolated func connect(
        ...
        try await self.deployment.pool(
...
extension Mongo.SessionPool {
    ...
    @inlinable public nonisolated func run<Command>(
...
extension Mongo.SessionPool {
    ...
    ///     `false` otherwise.
    private func yield(_ allocation: Allocation) -> Bool {
...
extension Mongo.SessionPool {
    ...
    func create(capabilities: Mongo.DeploymentCapabilities) async -> Allocation {
        guard case .filling = self.phase else {
        ...
        while let allocation: Allocation = self.released.popLast() {
            guard now < allocation.expiration(ttl: capabilities.sessions.ttl) else {
                continue
            }
        ...
        guard self.releasing.load(ordering: .relaxed) <= self.requests.count else {
    ...
    private func destroy(_ allocation: Allocation, reindex: Bool) {
        ...
        case .filling:
            ...
            guard self.yield(allocation) else {
                self.retained.remove(allocation.id)
