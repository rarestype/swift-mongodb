...
extension Mongo {
    public struct Transaction {
        @usableFromInline let session: Session
        @usableFromInline let pinned: ConnectionPool
...
extension Mongo.Transaction {
    func abort(with writeConcern: Mongo.WriteConcern?) async throws {
        try await self.run(
            command: Mongo.AbortTransaction.init(
    ...
    func commit(with writeConcern: Mongo.WriteConcern?) async throws {
        try await self.run(
            command: Mongo.CommitTransaction.init(
                writeConcern: writeConcern
            ),
            against: .admin
        )
...
extension Mongo.Transaction {
    @inlinable public var preconditionTime: BSON.Timestamp? {
        self.session.preconditionTime
    }
    @inlinable public var state: Mongo.TransactionState {
        self.session.transaction
    }

    @usableFromInline var deployment: Mongo.Deployment {
...
extension Mongo.Transaction {
    @inlinable public func run<Command>(
        ...
        return try await self.session.run(
    ...
    @inlinable public func run<Query, Success>(
        ...
        let batches: Mongo.Cursor<Query.Element> = try await self.session.begin(
            query: command,
            against: database,
            over: self.pinned,
            on: preference,
