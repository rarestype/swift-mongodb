...
extension Mongo.ConnectionPool {
    ...
    struct Allocations: Sendable {
        ...
        private var observers: Mongo.Observers
        /// All requests currently awaiting connections, identified by `UInt`.
        private var requests: [UInt: CheckedContinuation<Allocation, any Error>]
        ...
        private var counter: (connection: UInt, request: UInt)
...
extension Mongo.ConnectionPool.Allocations {
    mutating func whenEmpty(resume observer: CheckedContinuation<Void, Never>) {
    ...
    func destroy() {
        guard self.requests.isEmpty else {
            fatalError(
                "unreachable (deinitialized connection pool while continuations are awaiting!)"
            )
        }
        guard self.isEmpty else {
            fatalError(
                "unreachable (deinitialized connection pool while pool contains connections!)"
            )
        }
        guard case .draining = self.phase else {
...
extension Mongo.ConnectionPool.Allocations {
    ...
    ///     `false` otherwise.
    mutating func yield(_ allocation: Mongo.ConnectionPool.Allocation) -> Bool {
        switch self.requests.popFirst()?.value.resume(returning: allocation) {
        case nil: false
        case ()?: true
        }
    }
    mutating func fail(request: UInt, throwing error: any Error) {
...
extension Mongo.ConnectionPool.Allocations {
    ...
    mutating func reindex(_ allocation: __owned Mongo.ConnectionPool.Allocation) {
        ...
        guard   case nil = self.released.updateValue(
            allocation.channel,
            forKey: allocation.id
        ),
        channel === allocation.channel else {
    ...
    mutating func deindex(_ id: UInt) {
...
extension Mongo.ConnectionPool.Allocations {
    mutating func next(
        releasing: Int,
        settings: Mongo.ConnectionPoolSettings
    ) -> Mongo.ConnectionPool.AllocationResult {
        switch self.phase {
    ...
    private mutating func next() -> Mongo.ConnectionPool.Allocation? {
        guard let (id, channel): (UInt, any Channel) = self.released.popFirst() else {
            return nil
        }
...
extension Mongo.ConnectionPool.Allocations {
    mutating func replace(
        ...
        releasing: Int,
        settings: Mongo.ConnectionPoolSettings
    ) -> Mongo.ConnectionPool.Reservation? {
        if      case _? = self.released.removeValue(forKey: id) {
            ...
            guard   case nil = self.perished.update(with: id) else {
    ...
    private mutating func reserve(
        connector: Mongo.Connector<Mongo.Authenticator>,
        releasing: Int,
        settings: Mongo.ConnectionPoolSettings
    ) -> Mongo.ConnectionPool.Reservation? {
...
extension Mongo.ConnectionPool.Allocations {
    mutating func fill(with allocation: __owned Mongo.ConnectionPool.Allocation) {
        self.pending -= 1
    ...
    mutating func drain(throwing error: Mongo.ConnectionPoolDrainedError, erase: Bool = false) {
        if erase {
            self.pending -= 1
        }
    ...
    private func crosscancel(throwing error: Mongo.ConnectionPoolDrainedError) {
