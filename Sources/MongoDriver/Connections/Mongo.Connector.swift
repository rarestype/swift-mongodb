...
extension Mongo {
    struct Connector<Authenticator>: @unchecked Sendable {
        ...
        private let timeout: NetworkTimeout
...
extension Mongo.Connector {
    private var client: Mongo.Hello.ClientMetadata {
        .init(appname: self.appname)
    }

    private func channel() async throws -> any Channel {
...
extension Mongo.Connector<Never?> {
    func connect(interval: Milliseconds) async throws -> Mongo.MonitorServices {
        ...
        async let handshake: Mongo.Handshake = listener.run(hello: hello, by: deadline)
        ...
        do {
            ...
            async let sampler: Void = sampler.close()
...
extension Mongo.Connector<Mongo.Authenticator> {
    func connect(id: UInt) async throws -> Mongo.ConnectionPool.Allocation {
        ...
        switch await self.authenticator.establish(
            connection,
            client: self.client,
            by: deadline
        ) {
