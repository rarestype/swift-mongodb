...
extension Mongo {
    ...
    public final actor ConnectionPool {
        ...
        nonisolated let generation: UInt

        /// The size and width of this connection pool.
        public nonisolated let settings: ConnectionPoolSettings

        /// The host this pool creates connections to.
        public nonisolated let host: Host

        /// A handle for communicating back to the initiator of this pool.
        nonisolated let monitor: MonitorDelegate
        ...
        init(
            alongside monitor: MonitorDelegate,
...
extension Mongo.ConnectionPool {
    nonisolated func updateLatency(with nanoseconds: Nanoseconds) {
        self.networkLatency.store(nanoseconds, ordering: .relaxed)
    }
    nonisolated func recentLatency() -> Nanoseconds {
...
extension Mongo.ConnectionPool {
    ...
    private func drain(throwing error: Mongo.ConnectionPoolDrainedError) {
...
extension Mongo.ConnectionPool {
    ...
    @usableFromInline nonisolated func adjust(
        deadline: ContinuousClock.Instant
    ) -> Mongo.DeadlineAdjustments {
...
extension Mongo.ConnectionPool {
    nonisolated func log<Type>(event: Type) where Type: Mongo.MonitorEventType {
        self.logger?.yield(
            event: Mongo.MonitorEvent<Type>.init(
...
extension Mongo.ConnectionPool {
    ...
    /// before the deadline.
    private func fail(request id: UInt, by deadline: ContinuousClock.Instant) async throws {
        //  will throw ``CancellationError`` if request succeeds
        try await Task.sleep(until: deadline, clock: .continuous)
        self.allocations.fail(
            request: id, throwing: Mongo.ConnectionPoolTimeoutError.init(
                host: self.host
            )
        )
    }
...
extension Mongo.ConnectionPool {
    ...
    func create(by deadline: ContinuousClock.Instant) async throws -> Allocation {
        while true {
            ...
            case .blocked(let id):
                async let _: Void = self.fail(request: id, by: deadline)
    ...
    /// ``drain``.
    private func replace(perished allocation: Allocation) async {
        if  let reservation: Reservation = self.allocations.replace(
                allocation.id,
                releasing: self.releasing.load(ordering: .relaxed),
                settings: self.settings
            ) {
    ...
    private func fill(reservation: Reservation) async {
        ...
        do {
            ...
            self.allocations.drain(
                throwing: .init(because: error, host: self.host),
                erase: true
            )
