...
extension Mongo {
    ...
    /// `deinit`.
    public final class Connection: Identifiable {
        @usableFromInline internal let allocation: ConnectionPool.Allocation

        @usableFromInline internal let pool: ConnectionPool

        @usableFromInline internal var reuse: Bool

        private init(allocation: ConnectionPool.Allocation, pool: ConnectionPool) {
...
extension Mongo.Connection {
    ...
    public convenience init(
        from pool: Mongo.ConnectionPool,
...
extension Mongo.Connection {
    ...
    /// connection.
    @inlinable public var reusable: Bool {
        self.reuse
    }
    /// Interrupts this connection’s IO channel, and marks it as
    /// non-reusable.
    @_spi(testable) public func crosscancel(throwing error: any Error) {
