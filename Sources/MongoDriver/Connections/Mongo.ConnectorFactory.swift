...
extension Mongo {
    struct ConnectorFactory: Sendable {
        private let executors: any EventLoopGroup
...
extension Mongo.ConnectorFactory {
    func callAsFunction<Authenticator>(
        authenticator: Authenticator,
        timeout: Mongo.NetworkTimeout,
        host: Mongo.Host
    ) -> Mongo.Connector<Authenticator> {
        .init(
            authenticator: authenticator,
...
extension Mongo.ConnectorFactory {
    private func bootstrap(timeout: Milliseconds, host: Mongo.Host) -> ClientBootstrap {
        .init(group: self.executors)
            .channelOption(
                ChannelOptions.Types.ConnectTimeoutOption.init(),
                value: .milliseconds(timeout.rawValue)
            )
            .channelOption(
                ChannelOptions.Types.SocketOption.init(
