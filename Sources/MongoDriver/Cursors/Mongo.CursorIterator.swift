...
extension Mongo {
    ...
    public struct CursorIterator {
        public let id: CursorIdentifier
        ...
        public let preference: ReadPreference
        /// The database and collection this cursor iterates over.
        public let namespace: Namespaced<Collection>
        ...
        /// cursors.
        @usableFromInline let lifecycle: CursorLifecycle
        ...
        @usableFromInline let timeout: NetworkTimeout
...
extension Mongo.CursorIterator {
    @usableFromInline func deadline() -> ContinuousClock.Instant {
...
extension Mongo.CursorIterator {
    ...
    @inlinable public func get<Element>(
        more _: Element.Type
    ) async throws -> Mongo.CursorBatch<Element>
        where Element: BSONDecodable {
        try await self.pinned.session.run(
            command: Mongo.GetMore<Element>.init(
                cursor: self.id,
    ...
    @usableFromInline func kill() async throws -> Mongo.KillCursorsResponse {
        try await self.pinned.session.run(
