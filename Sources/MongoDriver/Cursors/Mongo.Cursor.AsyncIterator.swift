...
extension Mongo.Cursor {
    public final class AsyncIterator {
        @usableFromInline var cursor: Mongo.CursorIterator?
        @usableFromInline var first: [BatchElement]?

        init(
            cursor: Mongo.CursorIterator?,
            first: [BatchElement]
        ) {
            self.cursor = cursor
        ...
        deinit {
            guard case nil = self.cursor else {
                fatalError("""
...
extension Mongo.Cursor.AsyncIterator: AsyncIteratorProtocol {
    @inlinable public func next() async throws -> [BatchElement]? {
        if let first: [BatchElement] = self.first {
            self.first = nil
            return first
        }

        guard let cursor: Mongo.CursorIterator = self.cursor else {
            return nil
        }
