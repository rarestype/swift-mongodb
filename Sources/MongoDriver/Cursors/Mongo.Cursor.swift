...
extension Mongo {
    public struct Cursor<BatchElement> where BatchElement: BSONDecodable & Sendable {
        @usableFromInline let iterator: AsyncIterator

        private init(iterator: AsyncIterator) {
            self.iterator = iterator
        }
    }
}
extension Mongo.Cursor {
    public var cursor: Mongo.CursorIterator? {
        self.iterator.cursor
    }
}
extension Mongo.Cursor {
    @usableFromInline static func create(
    ...
    @usableFromInline func destroy() async {
...
extension Mongo.Cursor: AsyncSequence {
    public typealias Element = [BatchElement]
