...
extension Mongo.Authenticator {
    final actor Cache {
        private var sha256: [SCRAM.Challenge.CacheIdentifier: SCRAM.ClientResponse<SHA256>.Keys]
...
extension Mongo.Authenticator.Cache {
    private func store(
        _ keys: SCRAM.ClientResponse<SHA256>.Keys,
        for id: SCRAM.Challenge.CacheIdentifier
    ) {
        self.sha256[id] = keys
    }
}
extension Mongo.Authenticator.Cache {
    nonisolated func sha256(
        challenge: SCRAM.Challenge, password: String,
        received: SCRAM.Message,
        sent: SCRAM.Start
    ) async throws -> SCRAM.ClientResponse<SHA256> {
        ...
        let response: SCRAM.ClientResponse<SHA256> = try .init(
            cached: &keys,
            challenge: challenge,
            password: password,
            received: received,
