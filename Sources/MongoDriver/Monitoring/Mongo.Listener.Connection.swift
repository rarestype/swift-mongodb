...
extension Mongo.Listener.Connection {
    func run(
        hello: __owned Mongo.Hello,
        by deadline: ContinuousClock.Instant
    ) async throws -> Mongo.Handshake {
        ...
        let reply: Mongo.Reply = try await self.run(
            command: hello, against: .admin,
            by: deadline
        )
        let received: ContinuousClock.Instant = .now
        return .init(
            response: try .init(bson: reply()),
