...
extension Mongo {
    ...
    @frozen public struct DNS {
        public let host: Host

        @inlinable public init(_ host: Host) {
            self.host = host
        }
    }
}
extension Mongo.DNS: ExpressibleByArrayLiteral {
    @inlinable public init(arrayLiteral: String...) {
...
extension Mongo.DNS: Mongo.DiscoveryMode {
    @inlinable public static subscript(seed: Self) -> Mongo.SeedingMethod {
        .dns(seed)
    }
}
