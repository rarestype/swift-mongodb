...
extension Mongo.URI {
    ...
    @frozen public struct Location: Sendable {
        public let userinfo: (username: String, password: String)
        public let domains: Mongo.SeedingMethod
        public let path: Mongo.Database
...
extension Mongo.URI.Location: Mongo.ServiceLocator {
    public typealias Login = Mongo.User

    @inlinable public var database: Mongo.Database? { self.path }
}
