extension Mongo.URI {
    ...
    @frozen public struct Base<Login, Discovery>: Sendable
        where Login: Mongo.LoginMode, Discovery: Mongo.DiscoveryMode {
        public let userinfo: Login.Userinfo

        @inlinable public init(userinfo: Login.Userinfo) {
            self.userinfo = userinfo
        }
    }
}
extension Mongo.URI.Base<Mongo.Guest, Mongo.DirectSeeding> {
    @inlinable public var SRV: Mongo.URI.Base<Mongo.Guest, Mongo.DNS> {
        .init(userinfo: ())
    }
}
...
extension Mongo.URI.Base where Login == Mongo.Guest {
    @inlinable public static func / (
        base: Self,
        userinfo: Mongo.User.Userinfo
    ) -> Mongo.URI.Base<Mongo.User, Discovery> {
    ...
    @inlinable public static func / (
        base: Self,
        domains: Discovery.Seedlist
    ) -> Mongo.URI.Authority<Login> {
...
extension Mongo.URI.Base where Login == Mongo.User {
    ...
    @inlinable public static func * (
        base: Self,
        domains: Discovery.Seedlist
    ) -> Mongo.URI.Authority<Mongo.User> {
        .init(userinfo: base.userinfo, domains: Discovery[domains])
    }
    @inlinable public static func * (
        base: Self,
