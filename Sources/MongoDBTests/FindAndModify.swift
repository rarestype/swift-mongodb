...
@Suite struct FindAndModify: Mongo.TestBattery {
    ...
    func run(with pool: Mongo.SessionPool) async throws {
        ...
        states.0 = [
            ...
            .init(
                id: "Jeanine Áñez Chávez",
                party: "Demócratas",
                since: 2019
            ),

            .init(
                id: "Dina Ercilia Boluarte Zegarra",
                party: "Perú Libre",
                since: 2022
            ),

            .init(
                id: "Mary Elizabeth Truss",
                party: "Conservatives",
                since: 2022
            ),
        ]
        ...
        do {
            ...
            let (response, _): (Ruler?, Never?) = try await session.run(
                command: Mongo.FindAndModify<Mongo.Existing<Ruler>>.init(
                    self.collection,
            ...
            let rulers: [Ruler] = try await session.run(
                command: Mongo.Find<Mongo.SingleBatch<Ruler>>.init(
                    self.collection,
                    limit: 10
                ),
                against: self.database
        ...
        do {
            ...
            let (response, _): (Ruler?, Never?) = try await session.run(
                command: Mongo.FindAndModify<Mongo.Removing<Ruler>>.init(
                    self.collection,
                    writeConcern: .majority,
                    returning: .deleted
                ) {
