...
@Suite struct ChangeStreams: Mongo.TestBattery {
    ...
    func run(with pool: Mongo.SessionPool) async throws {
        ...
        var changes: [Mongo.Change<PlanDelta>] = [
            ...
            .update(
                .init(
                    updatedFields: .init(owner: nil, level: "y"),
                    id: state.0.id
                ),
                before: nil,
                after: nil
        ...
        try await session.run(
            ...
            for try await batch: [Mongo.ChangeEvent<PlanDelta>] in $0 {
                ...
                for event: Mongo.ChangeEvent<PlanDelta> in batch {
                    guard
                    let expected: Mongo.Change<PlanDelta> = changes.popLast() else {
                        #expect(event == nil)
                        return
                    }
