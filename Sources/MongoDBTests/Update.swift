...
@Suite struct Update: Mongo.TestBattery {
    ...
    func run(with pool: Mongo.SessionPool) async throws {
        ...
        do {
            ...
            let response: Mongo.UpdateResponse<Int> = try await session.run(
                command: Mongo.Update<Mongo.One, Int>.init(
                    self.collection,
        ...
        do {
            ...
            let response: Mongo.UpdateResponse<Int> = try await session.run(
                command: Mongo.Update<Mongo.Many, Int>.init(
                    self.collection,
                    writeConcern: .majority
                ) {
                    $0[.ordered] = false
        ...
        do {
            let expected: Mongo.UpdateResponse<Int> = .init(
                selected: 1,
                modified: 0,
