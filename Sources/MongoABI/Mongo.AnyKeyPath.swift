...
extension Mongo {
    @frozen public struct AnyKeyPath: Equatable, Hashable, Sendable {
        public var stem: BSON.Key

        @inlinable internal init(stem: BSON.Key) {
            self.stem = stem
        }
    }
}
extension Mongo.AnyKeyPath: RawRepresentable {
    ...
    @inlinable public var rawValue: String { self.stem.rawValue }
}
...
extension Mongo.AnyKeyPath: CustomStringConvertible {
    @inlinable public var description: String { "$\(self.stem)" }
}
...
extension Mongo.AnyKeyPath {
    ...
    @inlinable public static func / (self: Self, next: Self) -> Self {
