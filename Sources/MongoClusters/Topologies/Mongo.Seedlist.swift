extension Mongo {
    ...
    /// monitoring tasks.
    public struct Seedlist: Sendable {
        @usableFromInline internal let elements: [Host]

        @inlinable public init(host: Host) {
            self.elements = [host]
        }
        @inlinable internal init(uniqueHosts: [Host]) {
...
extension Mongo.Seedlist {
    @inlinable public init(hosts: some Sequence<Mongo.Host>) {
...
extension Mongo.Seedlist: RandomAccessCollection {
    ...
    @inlinable public subscript(index: Int) -> Mongo.Host {
        self.elements[index]
    }
}
extension Mongo.Seedlist {
    ...
    @inlinable public func callAsFunction(_ range: some RangeExpression<Int>) -> Self {
