extension Mongo.Topology {
    public struct Replicated {
        ...
        private var members: [
            Mongo.Host: Mongo.ServerDescription<Mongo.ReplicaSetMember, Owner>
        ]
        /// The current term of the replica set.
        private var term: Mongo.ReplicaSetTerm?
        /// The name of the replica set.
        private var name: String

        private var primary: Mongo.Host?

        private init(
            members: [Mongo.Host: Mongo.ServerDescription<Mongo.ReplicaSetMember, Owner>],
...
extension Mongo.Topology.Replicated: Sequence {
    public func makeIterator() -> Dictionary<
        Mongo.Host,
        Mongo.ServerDescription<Mongo.ReplicaSetMember, Owner>
    >.Iterator {
...
extension Mongo.Topology.Replicated {
    ...
    mutating func combine(
        update: Mongo.TopologyUpdate.Primary,
        peerlist: Mongo.Peerlist,
        host: Mongo.Host,
        add: (Mongo.Host) -> ()
    ) -> Mongo.ReplicaSetMember? {
        guard self.name == peerlist.set else {
    ...
    mutating func combine(
        update: Mongo.TopologyUpdate.Slave,
        peerlist: Mongo.Peerlist,
        host: Mongo.Host,
        add: (Mongo.Host) -> ()
    ) -> Mongo.ReplicaSetMember? {
