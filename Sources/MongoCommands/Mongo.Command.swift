...
extension Mongo {
    ...
    public protocol Command<Response>: Sendable {
...
extension Mongo.Command {
    /// Returns nil.
    @inlinable public var outline: Mongo.OutlineVector? { nil }
    /// Returns ``Mongo.MaxTime/computed``.
    @inlinable public var timeout: Mongo.MaxTime { .computed }
}
extension Mongo.Command where ReadConcern == Mongo.ReadConcern {
    @inlinable public var readConcernLabel: Mongo.ReadConcern?? {
        self.readConcern
    }
}
extension Mongo.Command where ReadConcern == Never {
    ...
    @inlinable public var readConcern: Never? {
        nil
    }
}
extension Mongo.Command where WriteConcern == Mongo.WriteConcern {
    @inlinable public var writeConcernLabel: Mongo.WriteConcern? {
        self.writeConcern
    }
}
extension Mongo.Command where WriteConcern == Never {
    ...
    @inlinable public var writeConcern: Never? {
        nil
    }
}
...
extension Mongo.Command<Void> {
    ...
    @inlinable public static func decode(reply _: BSON.DocumentDecoder<BSON.Key>) {
    }
}
