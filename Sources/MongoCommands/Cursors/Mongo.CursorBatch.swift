...
extension Mongo {
    @frozen public struct CursorBatch<
        Element
    >: Sendable where Element: BSONDecodable & Sendable {
        public let namespace: Mongo.Namespaced<Mongo.Collection>
...
extension Mongo.CursorBatch {
    @inlinable public var id: Mongo.CursorIdentifier? {
...
extension Mongo.CursorBatch: BSONDecodable, BSONDocumentDecodable {
    @inlinable public init(bson: BSON.DocumentDecoder<BSON.Key>) throws {
        self = try bson["cursor"].decode() {
            .init(
                namespace: try $0["ns"].decode(to: Mongo.Namespaced<Mongo.Collection>.self),
