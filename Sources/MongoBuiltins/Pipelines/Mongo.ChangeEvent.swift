...
extension Mongo {
    @frozen public struct ChangeEvent<Delta> where Delta: MasterCodingDelta,
        Delta.Model: Identifiable {
        public let id: ChangeEventIdentifier
        public let clusterTime: BSON.Timestamp
        public let change: Change<Delta>

        @inlinable public init(
            id: ChangeEventIdentifier,
...
extension Mongo.ChangeEvent: BSONDocumentDecodable, BSONDecodable where
    ...
    @inlinable public init(bson: BSON.DocumentDecoder<CodingKey>) throws {
        ...
        case .update:
            change = .update(
                .init(
                    try bson[.updateDescription].decode(),
