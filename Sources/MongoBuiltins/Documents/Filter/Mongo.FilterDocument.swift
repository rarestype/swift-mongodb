...
extension Mongo {
    ...
    @frozen public struct FilterDocument: Mongo.EncodableDocument, Sendable {
...
extension Mongo.FilterDocument {
    @inlinable public static func `let`(
        _ variable: Mongo.Variable<some Any>,
        with populate: (inout Self) throws -> ()
    ) rethrows -> Self {
        try .let(variable.name, with: populate)
    }
    @inlinable public static func `let`(
        _ variable: Mongo.Variable<Any>,
        with populate: (inout Self) throws -> ()
    ) rethrows -> Self {
        try .let(variable.name, with: populate)
    }

    @inlinable internal static func `let`(
        _ variable: BSON.Key,
        with populate: (inout Self) throws -> ()
    ) rethrows -> Self {
...
extension Mongo.FilterDocument {
    @inlinable public subscript<Encodable>(key: Argument) -> Encodable?
