...
extension Mongo {
    @frozen public struct ProjectionEncoder<
        CodingKey
    >: Sendable where CodingKey: RawRepresentable<
            String
        > {
        @usableFromInline var bson: BSON.DocumentEncoder<CodingKey>

        @inlinable internal init(bson: BSON.DocumentEncoder<CodingKey>) {
            self.bson = bson
        }
    }
}
extension Mongo.ProjectionEncoder: BSON.Encoder {
    @inlinable public init(_ output: consuming BSON.Output) { self.init(bson: .init(output)) }
...
extension Mongo.ProjectionEncoder {
    ...
    @inlinable public subscript(path: CodingKey) -> Int? {
        get { nil }
        set {     }
    }

    @inlinable public subscript(path: CodingKey) -> Bool? {
        get { nil }
        set (value) { value?.encode(to: &self.bson[with: path]) }
    }

    /// Encodes a nested projection document.
    @inlinable public subscript<NestedKey>(
        path: CodingKey,
        using _: NestedKey.Type = NestedKey.self,
        yield: (inout Mongo.ProjectionEncoder<NestedKey>) -> ()
    ) -> Void {
        mutating get {
    ...
    @inlinable public subscript<ProjectionDocument>(path: CodingKey) -> ProjectionDocument?
    ...
    @inlinable public subscript(
        path: CodingKey,
        yield: (inout Mongo.ProjectionOperatorEncoder) -> ()
    ) -> Void {
        mutating get {
    ...
    @inlinable public subscript<Operator>(path: CodingKey) -> Operator?
