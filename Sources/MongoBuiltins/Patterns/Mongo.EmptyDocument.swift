...
extension Mongo {
    ...
    @frozen public struct EmptyDocument {
        @inlinable init() {
        }
    }
}
extension Mongo.EmptyDocument: ExpressibleByDictionaryLiteral {
    @inlinable public init(dictionaryLiteral _: (Never, Never)...) {}
}
extension Mongo.EmptyDocument: BSONEncodable, BSONDocumentEncodable {
    @inlinable public func encode(to _: inout BSON.DocumentEncoder<BSON.Key>) {
    }
}
extension Mongo.EmptyDocument: BSONDecodable, BSONKeyspaceDecodable {
    @inlinable public init(bson: consuming BSON.KeyspaceDecoder<BSON.Key>) throws {
